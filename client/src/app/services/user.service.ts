...
import { UserCredentials, LoginResponse, User, UserAndCompany } from '@models';
...
export class UserService {
  ...
  public async addUser(user: User): Promise<User> {
    const updatedUser = await this.http.post<User[]>(`/api/database/users`, user).toPromise();

    return updatedUser && updatedUser.length ? updatedUser[0] : null;
  }

  public async editUser(user: User | UserAndCompany): Promise<User> {
    const updatedUser = await this.http.patch<User[]>(`/api/database/users/${user.userId}`, user).toPromise();

    return updatedUser && updatedUser.length ? updatedUser[0] : null;
  }

  public async deleteUser(userId: number): Promise<User> {
    const deletedUser = await this.http.delete<User[]>(`/api/database/users/${userId}`).toPromise();

    return deletedUser && deletedUser.length ? deletedUser[0] : null;
  }

  public async logInUser(userCredentials: UserCredentials): Promise<LoginResponse> {
    const loginResponse = await this.http.post<LoginResponse>(`/api/auth/login`, userCredentials).toPromise();

    this.user = loginResponse?.success ? loginResponse.userAndCompany : null;
    return loginResponse;
  }

  public async resetPassword(token: string, password: string, verifyPassword: string): Promise<boolean> {
