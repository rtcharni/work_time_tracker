...
import { UsersService, AuthService, Queries } from "../services";
      ...
      async (req: Request, res: Response, next: NextFunction) => {
        try {
          ...
          if (loginResponse.success) {
            ...
            const token = TokenManagement.generateLoginToken(
        ...
        try {
          ...
          if (user[0].resetPasswordToken) {
            try {
              ...
              console.log(
                `User with ID ${user[0].userId} has already requested reset password in the past 30 min`
              );
              res.send(true);
            } catch (error) {
