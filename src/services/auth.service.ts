...
export class AuthService {
  public static async handleUserResetPassword(
    token: string,
    password: string,
    ...
    try {
      ...
      if (
        user.length !== 1 ||
        token !== user[0].resetPasswordToken ||
      ...
      await Queries.editGenericFields(
      ...
      console.error(
        `Error while handling user reset password with token: ${token}`
      );
      console.error(error);
      return false;
    }
